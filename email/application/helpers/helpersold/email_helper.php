...
if (!function_exists('facultyverifyVideo')) {
    function facultyverifyVideo($data=[])
    {
        ...
        $pu_approver = 'rnegi.saneden@gmail.com';
        ...
        $CI->email->from($from_email, $from_name);
        $CI->email->to($data['email']);  // replace it with receiver mail id
        $CI->email->subject('Notification - Video Approval Request'); // replace it with relevant subject

        $body = $CI->load->view('email_templates/tu_approval_video', ['data' => $data], true);
