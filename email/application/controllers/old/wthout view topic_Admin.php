...
$rs = $this->lib_model->Counter('m_role_assignment', array('fid' => $this->session->EmpId, 'sid'=> 3, 'status' => 0));
	...
	public function Auth()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$rs = $this->lib_model->Select('m_faculty', 'id,name', array('email' => $username, 'password' => $password, 'status' => 0));
			if (count($rs) == 1) {
				foreach ($rs as $r)
				{
					$Sessions = array(
...
/*
	...
	{
		$data['rs']  = $this->lib_model->Select('m_role', 'id,role', array('status' => 0));
		$data['rs2'] = $this->lib_model->Select('m_faculty', 'id,email,name', array('status' => 0));
		$this->load->view('admin/header', $data);
